...
-- =============================================
-- survey_question rows
-- We need section IDs. Since INSERT OR IGNORE may skip if rows exist,
-- we get IDs dynamically via subquery.
-- =============================================

-- AN-TOAN-CHECK: Section 1 (select questions)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","scale_labels_vi","scale_labels_en","required","dimension"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='an-toan-check' AND "order_idx"=0
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, q.scale_labels_vi, q.scale_labels_en, 1, q.dimension
FROM sec, (VALUES
  ('at_q1', 0, 'select',
  ...
  ('at_q5', 4, 'select',
   ...
   'safety')
) AS q(question_id,order_idx,type,label_vi,label_en,placeholder_vi,placeholder_en,scale_labels_vi,scale_labels_en,dimension);

-- AN-TOAN-CHECK: Section 2 (textarea open-ended)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","required"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='an-toan-check' AND "order_idx"=1
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, 1
FROM sec, (VALUES
  ('at_open1', 0, 'textarea',
...
) AS q(question_id,order_idx,type,label_vi,label_en,placeholder_vi,placeholder_en);

-- MARKETING-CHECK: Section 1 (select questions)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","scale_labels_vi","scale_labels_en","required","dimension"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='marketing-check' AND "order_idx"=0
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, q.scale_labels_vi, q.scale_labels_en, 1, q.dimension
FROM sec, (VALUES
...
-- MARKETING-CHECK: Section 2 (textarea open-ended)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","required"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='marketing-check' AND "order_idx"=1
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, 1
FROM sec, (VALUES
  ('mkt_open1', 0, 'textarea',
...
-- CSKH-CHECK: Section 1 (select questions)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","scale_labels_vi","scale_labels_en","required","dimension"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='cskh-check' AND "order_idx"=0
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, q.scale_labels_vi, q.scale_labels_en, 1, q.dimension
FROM sec, (VALUES
  ...
  ('cskh_q4', 3, 'select',
   ...
   'service'),
  ('cskh_q5', 4, 'select',
   'Nhân viên có được train để "thấu cảm" với bệnh nhân (put yourself in their shoes) không?',
   'Are staff trained to empathize with patients (put yourself in their shoes)?',
   NULL, NULL,
...
-- CSKH-CHECK: Section 2 (textarea open-ended)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","required"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='cskh-check' AND "order_idx"=1
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, 1
FROM sec, (VALUES
  ('cskh_open1', 0, 'textarea',
...
-- VAN-HOA-CHECK: Section 1 (select questions)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","scale_labels_vi","scale_labels_en","required","dimension"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='van-hoa-check' AND "order_idx"=0
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, q.scale_labels_vi, q.scale_labels_en, 1, q.dimension
FROM sec, (VALUES
  ...
  ('vh_q4', 3, 'select',
   ...
   'culture'),
  ('vh_q5', 4, 'select',
   'An toàn tâm lý (psychological safety) trong phòng khám như thế nào? Nhân viên có dám nói thẳng khi có vấn đề không?',
   'What is the psychological safety level in your clinic? Do staff dare to speak up when there is an issue?',
   NULL, NULL,
...
-- VAN-HOA-CHECK: Section 2 (textarea open-ended)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","required"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='van-hoa-check' AND "order_idx"=1
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, 1
FROM sec, (VALUES
...
-- THUONG-HIEU-CHECK: Section 1 (select questions)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","scale_labels_vi","scale_labels_en","required","dimension"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='thuong-hieu-check' AND "order_idx"=0
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, q.scale_labels_vi, q.scale_labels_en, 1, q.dimension
FROM sec, (VALUES
...
-- THUONG-HIEU-CHECK: Section 2 (textarea open-ended)
INSERT OR IGNORE INTO "survey_question" (
  "section_id","question_id","order_idx","type","label_vi","label_en",
  "placeholder_vi","placeholder_en","required"
) WITH sec AS (
  SELECT id FROM "survey_section" WHERE "survey_id"='thuong-hieu-check' AND "order_idx"=1
)
SELECT sec.id, q.question_id, q.order_idx, q.type, q.label_vi, q.label_en,
       q.placeholder_vi, q.placeholder_en, 1
FROM sec, (VALUES
