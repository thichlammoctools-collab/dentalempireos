...
async function sendScannerAiCompleteEmail(db, responseId, type) {
  const row = await db.prepare("SELECT email, clinic_name, lang FROM scanner_response WHERE id = ?").bind(responseId).first();
  if (!row?.email) {
  ...
  const title = isAnalysis ? lang === "vi" ? "Bản soi chiếu hệ thống đã sẵn sàng" : "Your AI Analysis is Ready" : lang === "vi" ? "Kế hoạch 30 ngày đã được tạo" : "Your 30-Day Plan is Ready";
  const subtitle = isAnalysis ? lang === "vi" ? "Bản phân tích AI của bạn đã hoàn thành. Nhấn để xem chi tiết." : "Your AI analysis has been generated. Tap to view the results." : lang === "vi" ? "Kế hoạch hành động 30 ngày của bạn đã sẵn sàng. Nhấn để xem chi tiết." : "Your 30-day action plan is ready. Tap to view the details.";
            ...
            <td style="background: linear-gradient(135deg, #1e1e2e 0%, #13131f 100%); border: 1px solid rgba(34,211,238,0.15); border-radius: 16px; padding: 40px 36px;">
              ...
              <div style="text-align: center; margin-bottom: 32px;">
                <a href="${resultUrl}"
                  ...
                  ${isAnalysis ? lang === "vi" ? "📖 Xem bản phân tích" : "📖 View Analysis" : lang === "vi" ? "📅 Xem kế hoạch" : "📅 View Plan"} →
                </a>
              </div>

              <!-- Note -->
              <p style="margin: 0; font-size: 13px; color: #475569; line-height: 1.6; text-align: center;">
                ${lang === "vi" ? "Kết quả được cá nhân hóa dựa trên dữ liệu của phòng khám bạn đã cung cấp." : "Results are personalized based on the data your clinic provided."}
              </p>
...
export {
  sendWelcomeEmail as a,
  sendScannerAiCompleteEmail as s
};
