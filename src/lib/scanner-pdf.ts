...
function drawScoreDashboard(
  ...
  ctx.page.drawText(sanitizePdfText(totalLevel.label_vi, ctx.fontBold), {
  ...
  for (const para of paragraphs) {
    ...
    for (const word of words) {
      ...
      try {
        ...
        current += current ? ` ${sanitizePdfText(word, font)}` : sanitizePdfText(word, font);
      }
    }
...
export async function generateScannerPdf(
  ...
  const reportTitleLines = wrapText(reportTitle, fontBold, 28, CONTENT_WIDTH);
  reportTitleLines.slice(0, 2).forEach((line, index) => cover.drawText(line, {
    x: 50, y: PAGE_HEIGHT - 145 - index * 32, size: 28,
  ...
  const drawCoverValue = (label: string, value: string, bold = false) => {
    const lines = wrapText(value, bold ? fontBold : font, 11, PAGE_WIDTH - 250);
    cover.drawText(label, { x: 50, y: infoY, size: 10, font, color: MUTED });
    lines.forEach((line, index) => cover.drawText(line, {
      x: 200, y: infoY - index * 14, size: 11, font: bold ? fontBold : font, color: TEXT,
    }));
    infoY -= Math.max(22, lines.length * 14 + 8);
  };
  drawCoverValue(t.clinic, response.clinic_name ?? '—', true);

  if (response.owner_name) {
    drawCoverValue(t.owner, response.owner_name, true);
  }
  if (response.clinic_address) {
    drawCoverValue(t.address, response.clinic_address);
  }
  if (response.years_in_operation !== null) {
    drawCoverValue(t.years, String(response.years_in_operation));
  }
  if (response.staff_count !== null) {
    drawCoverValue(t.staff, String(response.staff_count));
  }
  if (identity?.phone) {
    drawCoverValue(lang === 'vi' ? 'Điện thoại' : 'Phone', identity.phone);
  }

  drawCoverValue(t.date, response.created_at.slice(0, 10), true);
...
function renderMarkdownToPdf(ctx: PdfContext, markdown: string) {
  ...
  for (const tok of tokens) {
    if (tok.type === 'heading') {
      const text = stripMarkdown(tok.text.replace(/[*_`#]/g, ''));
