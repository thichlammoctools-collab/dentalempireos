...
export async function listCategories(db: D1Database): Promise<BlogCategory[]> {
  const { results } = await db
    .prepare('SELECT * FROM "blog_category" ORDER BY "sort_order", "name"')
    .all<BlogCategory>();
  return results;
}
...
export async function upsertCategory(db: D1Database, input: BlogCategoryInput): Promise<void> {
  ...
  await db
    .prepare(
      `INSERT INTO "blog_category" ("id","name","slug","description","icon","color","sort_order","post_count","created_at")
       VALUES (?,?,?,?,?,?,?,0,?)
