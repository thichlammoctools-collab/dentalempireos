...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (status && !consultationStatusValues.includes(status as ConsultationStatus)) {
    return badRequest('Trạng thái không hợp lệ.');
  }
  return json(await listConsultationRequests(env.DB, status as ConsultationStatus | undefined));
};
